<?php
	...
	if( isset($action) && !empty($action) && $action == "แก้ไข | Edit" && eregi($_SERVER['HTTP_HOST'],$_SERVER['HTTP_REFERER']) ){
		...
		$site_email_from_name = trim(mosGetParam($_FORM,'site_email_from_name',''));
		$site_email_from = trim(mosGetParam($_FORM,'site_email_from',''));

		$qryUpdate = "update $_Config_table[site] set
		site_email_from_name = $DB->qstr('$site_email_from_name'),
		site_email_from = $DB->qstr('$site_email_from')
		where site_id = '1' ";
		$DB->Execute($qryUpdate);
		mosRedirect("emailSystem.php");

	}

	$qry="select site_email_from_name, site_email_from from $_Config_table[site] where site_id = '1' ";
	$rs = $DB->Execute($qry);
	$row = $rs->FetchRow() ;
?>
...
<div class="updated" id="Show-result"><p><span class="headSm">เกิดข้อผิดพลาด : </span><span class="date"><?php echo "รูปประกอบขนาดเล็ก : ".$errMsg; if($errMsg !="" && $errMsg2 !="" ){ echo ", "; } echo "รูปประกอบขนาดใหญ่ : ".$errMsg2;?></span></p></div>
    ...
    <table class="form-table">
            <tbody>
            <tr class="form-field">
              <th align="left" valign="top" scope="row">ชื่ออีเมล์</th>
              <td align="left" valign="top" class="idata"><input name="site_email_from_name" type="text" id="site_email_from_name" size="40" value="<?php echo $row["site_email_from_name"];?>" /></td>
            </tr>
            <tr class="form-field">
              <th align="left" valign="top" scope="row">อีเมล์</th>
              <td align="left" valign="top" class="idata"><input name="site_email_from" type="text" id="site_email_from" size="40" value="<?php echo $row["site_email_from"];?>" /></td>
            </tr>
            <tr class="form-field">
              <th align="left" valign="top" scope="row">&nbsp;</th>
              <td align="left" valign="top" class="idata"><span class="description">กำหนดเพื่อใช้ในการส่งเมล์ออกจากเวบไซต์ไปยังลูกค้า <br />
              เช่นในกรณี ติดต่อเรา และในการรับอีเมล์แจ้งเตือนต่างๆ</span></td>
            </tr>
          </tbody>
    </table>
    <p class="submit">
    <input type="submit" value="แก้ไข | Edit" name="action" class="button-primary">
    <input type="button" name="action2" value="ยกเลิก | Cancel" class="button-primary" onclick="MM_goToURL('parent','emailSystem.php');return document.MM_returnValue" /></p>
    </form>
</div>
</body>
</html>
