		...
		function checkOccupation()
		{
		 ...
		 function checkFrm()
		 {
			with(document.formCustomer)
			{
				...
				if(Trim(hint.value) == '')
				{
					alert('กรุณาใส่คำถามกันลืม');
					hint.focus();
					return false;
				}
				if(Trim(ans.value) == '')
				{
					alert('กรุณาใส่คำตอบกันลืม');
					ans.focus();
					return false;
				}
				if(enews[0].checked == false &&  enews[1].checked == false )
				{
					alert('กรุณาเลือกการรับข่าวสาร');
					gender[0].focus();
					return false;
				}
